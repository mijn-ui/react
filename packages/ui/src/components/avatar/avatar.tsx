...
export const avatarStyles = cva(["relative h-10 w-10 shrink-0 overflow-hidden rounded-full"], {
  variants: {
    size: {
...
const AvatarGroup = ({ max, children, className, unstyled = false, ...props }: AvatarGroupProps) => {
...
type AvatarProps = React.ComponentPropsWithRef<typeof AvatarPrimitive.Root> & AvatarVariantProps & UnstyledProps;

const Avatar = ({ unstyled, size, className, ...props }: AvatarProps) => {
  const { unstyled: contextUnstyled } = useUnstyled();
  const isUnstyled = unstyled ?? contextUnstyled;

  return <AvatarPrimitive.Root className={applyUnstyled(isUnstyled, avatarStyles({ size }), className)} {...props} />;
};
...
const AvatarImage = ({ unstyled, className, ...props }: AvatarImageProps) => {
  ...
  return (
    <AvatarPrimitive.Image
      className={applyUnstyled(isUnstyled, "aspect-square h-full w-full object-cover", className)}
      {...props}
    />
  );
};
...
const AvatarFallback = ({ unstyled, className, ...props }: AvatarFallbackProps) => {
  const { unstyled: contextUnstyled } = useUnstyled();
  const isUnstyled = unstyled ?? contextUnstyled;
