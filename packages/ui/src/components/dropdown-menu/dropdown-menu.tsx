...
type DropdownTriggerProps = React.ComponentPropsWithRef<typeof DropdownMenuPrimitive.Trigger> & UnstyledProps;

const DropdownMenuTrigger = ({ unstyled, className, ...props }: DropdownTriggerProps) => {
...
const DropdownMenuSubTrigger = ({ unstyled, className, inset, children, ...props }: DropdownMenuSubTriggerProps) => {
  const { unstyled: contextUnstyled } = useUnstyled();
  const isUnstyled = unstyled ?? contextUnstyled;
...
type DropdownMenuSubContentProps = React.ComponentPropsWithRef<typeof DropdownMenuPrimitive.SubContent> & UnstyledProps;

const DropdownMenuSubContent = ({ unstyled, className, ...props }: DropdownMenuSubContentProps) => {
  const { unstyled: contextUnstyled } = useUnstyled();
  const isUnstyled = unstyled ?? contextUnstyled;
...
type DropdownMenuContentProps = React.ComponentPropsWithRef<typeof DropdownMenuPrimitive.Content> & UnstyledProps;

const DropdownMenuContent = ({ className, unstyled, sideOffset = 4, ...props }: DropdownMenuContentProps) => {
  const { unstyled: contextUnstyled } = useUnstyled();
  const isUnstyled = unstyled ?? contextUnstyled;
...
const DropdownMenuItem = ({ unstyled, className, inset, ...props }: DropdownMenuItemProps) => {
  const { unstyled: contextUnstyled } = useUnstyled();
  const isUnstyled = unstyled ?? contextUnstyled;
...
const DropdownMenuCheckboxItem = ({
  ...
  return (
    <DropdownMenuPrimitive.CheckboxItem
      ...
      {...props}
    >
      <span className={applyUnstyled(isUnstyled, "absolute left-2 flex h-3.5 w-3.5 items-center justify-center")}>
...
type DropdownMenuRadioItemProps = React.ComponentPropsWithRef<typeof DropdownMenuPrimitive.RadioItem> & UnstyledProps;

const DropdownMenuRadioItem = ({ className, unstyled, children, ...props }: DropdownMenuRadioItemProps) => {
  const { unstyled: contextUnstyled } = useUnstyled();
  const isUnstyled = unstyled ?? contextUnstyled;
...
const DropdownMenuLabel = ({ className, unstyled, inset, ...props }: DropdownMenuLabelProps) => {
  const { unstyled: contextUnstyled } = useUnstyled();
  const isUnstyled = unstyled ?? contextUnstyled;

  return (
    <DropdownMenuPrimitive.Label
      className={applyUnstyled(isUnstyled, cn("px-2 py-1.5 text-sm font-semibold", inset && "pl-8"), className)}
      {...props}
    />
  );
};
...
type DropdownMenuSeparatorProps = React.ComponentPropsWithRef<typeof DropdownMenuPrimitive.Separator> & UnstyledProps;

const DropdownMenuSeparator = ({ className, unstyled, ...props }: DropdownMenuSeparatorProps) => {
  const { unstyled: contextUnstyled } = useUnstyled();
  const isUnstyled = unstyled ?? contextUnstyled;
...
const DropdownMenuShortcut = ({
  ...
  return (
    <span className={applyUnstyled(isUnstyled, "ml-auto text-xs tracking-widest opacity-60", className)} {...props} />
  );
};
